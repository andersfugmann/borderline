...
let rec process_rule table (rules, target) =
  let gen_op table target = function
    ...
    | Filter(dir, TcpPort(ports)) -> [ ( [(Ir.Protocol(Ir.TCP), true); (Ir.TcpPort(dir, ports), true)], target ) ]
    | Filter(dir, UdpPort(ports)) -> [ ( [(Ir.Protocol(Ir.UDP), true); (Ir.UdpPort(dir, ports), true)], target ) ]
